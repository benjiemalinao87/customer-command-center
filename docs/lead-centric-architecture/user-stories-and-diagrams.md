...
graph TB
    ...
    subgraph "🔄 Processing Layer"
        subgraph "Webhook Processors"
            CW[Cloudflare Worker<br/>Smart Logic]
            NB[Node.js Backend<br/>Legacy Support]
            SE[Supabase Edge<br/>Real-time]
        end
    ...
    subgraph "💾 Data Layer"
        subgraph "Supabase Database"
            ...
            LE[leads]
            LA[lead_activities]
            LPS[lead_pipeline_stages]
            LCF[lead_custom_fields]
            LFD[lead_field_definitions]
        end
    end
    ...
    subgraph "🎨 Presentation Layer"
        subgraph "React Frontend"
            ...
            LV[Lead Views]
            AH[Activity History]
            PM[Pipeline Management]
            RP[Reports]
        end
    end

    subgraph "🚀 API Layer"
        V3[API v3<br/>Cloudflare Workers]
        V2[Legacy APIs<br/>Node.js]
    end
    ...
    classDef presentNode fill:#fff3e0
    classDef apiNode fill:#fce4ec

    class WF,LP,FB,GA,PC,WI,RF,CRM sourceNode
    class CW,NB,SE,CC,LD,AT,VS processNode
    class CT,LE,LA,LPS,LCF,LFD dataNode
    class DS,LV,AH,PM,RP presentNode
    class V3,V2 apiNode
```
...
stateDiagram-v2
    ...
    New --> Qualified : Agent Call<br/>Webhook Update<br/>Form Engagement
    New --> Lost : No Response<br/>Not Interested<br/>Budget Mismatch
    New --> Inactive : Timeout<br/>No Engagement
    
    Qualified --> Proposal : Site Visit<br/>Requirements Met<br/>Budget Confirmed
    Qualified --> Lost : Requirements Not Met<br/>Budget Issues<br/>Competition
    Qualified --> Inactive : Delayed Decision<br/>Seasonal Hold
    
    Proposal --> Won : Contract Signed<br/>Deposit Received<br/>Project Approved
    Proposal --> Lost : Proposal Rejected<br/>Price Too High<br/>Competitor Chosen
    Proposal --> Qualified : Revision Needed<br/>More Information<br/>Re-evaluation
    
    Won --> [*] : Project Complete<br/>Revenue Recognized
    Lost --> [*] : Closed<br/>Archived
    Inactive --> New : Re-engaged<br/>Follow-up Success
    Inactive --> Lost : Permanent Loss<br/>Contact Unreachable

    %% Activity Triggers
    New : Entry Activities
    New : - Lead Created
    New : - Welcome Email
    New : - Agent Assignment

    Qualified : Engagement Activities
    Qualified : - Discovery Call
    Qualified : - Needs Assessment
    Qualified : - Budget Discussion

    Proposal : Sales Activities
    Proposal : - Quote Generation
    Proposal : - Presentation
    Proposal : - Negotiation

    Won : Fulfillment Activities
    Won : - Contract Processing
    Won : - Project Kickoff
    Won : - Delivery Management
...
```mermaid
flowchart TD
    WH[📥 Webhook Payload Received] --> CE{🔍 Contact Exists?}
    
    CE -->|CRM ID Match| EC[📋 Existing Contact]
    CE -->|Phone Match| EC
    CE -->|Email Match| EC
    CE -->|No Match| NC[🆕 New Contact]
    
    NC --> CCL[Create Contact<br/>& Lead]
    CCL --> CAC[Create Activity<br/>Log Creation]
    CAC --> SR[✅ Success Response]
    
    EC --> LE{🎯 Lead Exists?<br/>Same Product?}
    
    LE -->|No Lead Found| CNL[🆕 Create New Lead]
    LE -->|Lead Exists| DA[🧠 Decision Algorithm]
    
    CNL --> CAL[Create Activity<br/>Log Creation]
    CAL --> SR
    
    DA --> SK{📊 Status Keyword?<br/>qualified/converted/won}
    SK -->|No| CNL
    SK -->|Yes| TG{⏰ Time Gap?<br/>< 30 days}
    
    TG -->|No - Old Lead| CNL
    TG -->|Yes - Recent| CS{🔄 Conversion Status?<br/>Already Won?}
    
    CS -->|Already Won| CNL
    CS -->|Still Active| UL[🔄 Update Existing Lead]
    
    UL --> UA[Update Activities<br/>- Status Change<br/>- Appointment<br/>- Conversion]
    UA --> SR
    
    %% Example Scenarios
    E1[Example: John Doe<br/>Kitchen → Solar<br/>= NEW LEAD]
    E2[Example: John Doe<br/>Kitchen New → Qualified<br/>= UPDATE LEAD]
    E3[Example: John Doe<br/>Kitchen after 31 days<br/>= NEW LEAD]
    
    %% Styling
    classDef decisionNode fill:#fff2cc,stroke:#d6b656
    classDef actionNode fill:#d5e8d4,stroke:#82b366
    classDef terminalNode fill:#f8cecc,stroke:#b85450
    classDef exampleNode fill:#e1d5e7,stroke:#9673a6
    
    class CE,LE,DA,SK,TG,CS decisionNode
    class CCL,CNL,UL,CAC,CAL,UA actionNode
    class SR terminalNode
    class E1,E2,E3 exampleNode
```
